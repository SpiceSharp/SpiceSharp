...
namespace Sandbox
{
    public partial class Main : Form
    {
        ...
        public Main()
        {
            ...
            Series ns = chMain.Series.Add("Noise");
            ...
            {
                double outpn = 10.0 * Math.Log10(data.GetOutputNoiseDensity());
                ns.Points.AddXY(data.GetFrequency(), outpn);

                double outrn = 10.0 * Math.Log10(reference_out[index++]);
                rs.Points.AddXY(data.GetFrequency(), outrn);
            };
            nr.Netlist.Simulate();
