                        ...
                        switch (at.Name.kind)
                        {
                            case BRACKET:
                                ...
                                if (bt.Name.image.ToLower() == "v" && bt.Parameters.Length == 1 && ReaderExtension.IsNode(bt.Parameters[0]))
                                    netlist.Circuit.Nodes.Nodeset.Add(new Identifier(bt.Parameters[0].image), netlist.ParseDouble(at.Value));
                                else
                            ...
                            default:
                                if (ReaderExtension.IsNode(at.Name))
                                    netlist.Circuit.Nodes.Nodeset.Add(new Identifier(at.Name.image), netlist.ParseDouble(at.Value));
                                else
