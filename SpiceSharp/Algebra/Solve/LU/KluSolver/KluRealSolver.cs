...
/// <summary>
/// Solves sparse sets of equations with real numbers the way KLU does: the sparsity pattern
...
public partial class KluRealSolver : KluSolver<double>
{
    ...
    private void EnsureValueCapacity()
    {
        ...
        if (_values == null || _values.Length < count)
            _values = new double[Math.Max(count, 4)];
    }
    ...
    private void Allocate(int size)
    {
        ...
        _intermediate = new double[capacity];
        _visitFlag = 0;

        int guess = Math.Max(4 * NonZeroCount, 16);
        _lowerRows = new int[guess];
        _lowerValues = new double[guess];
        _upperRows = new int[guess];
        _upperValues = new double[guess];
        _offRows = new int[Math.Max(NonZeroCount, 4)];
        _offValues = new double[Math.Max(NonZeroCount, 4)];
    }
    ...
    private void ApplyScaling(int size)
    {
        ...
        if (scaling == KluScaling.Sum)
        {
            for (int p = 0; p < count; p++)
                _rowScaleReciprocal[rows[p]] += Math.Abs(_values[p]);
        }
        else
        {
            for (int p = 0; p < count; p++)
            {
                double magnitude = Math.Abs(_values[p]);
        ...
        for (int p = 0; p < count; p++)
            _values[p] *= _rowScaleReciprocal[rows[p]];
    }
    ...
    private int FactorSingleton(int position, double absolute)
    {
        ...
        if (!found || Math.Abs(pivot) <= absolute)
            return position;
    ...
    private int FactorBlock(int from, int to, double relative, double absolute)
    {
        ...
        for (int j = 0; j < size; j++)
        {
            ...
            for (int c = 0; c < candidateCount; c++)
            {
                ...
                double magnitude = Math.Abs(_work[row]);
    ...
    /// </summary>
    /// <remarks>
    /// The route is only worth reusing while the pivots it picked are still good ones. Every
    /// column is therefore held to the same standard the full factorization would have
    /// applied: the pivot has to remain large compared to the rest of its column. Reporting
    /// failure the moment that stops being true is what keeps reusing the route safe, because
    /// a pivot that has quietly degraded produces a factorization that is numerically useless
    /// while still looking like it succeeded.
    /// </remarks>
    /// <returns>
    /// <c>true</c> if the route still works; <c>false</c> if a pivot has degraded, in which
        ...
        for (int block = 0; block < symbolic.Blocks; block++)
        {
            ...
            for (int position = from; position < to; position++)
            {
                ...
                for (int entry = _entryColumnStart[position]; entry < entryEnd; entry++)
                {
                    ...
                    double value = _values[_entrySource[entry]];
                    if (target >= 0)
                        _work[target] = value;
                    else
                ...
                for (int p = start; p < end; p++)
                {
                    ...
                    double value = _work[row];
                    _work[row] = 0.0;
                    _lowerValues[p] = value;
                    double candidate = Math.Abs(value);
    ...
    private void AddLower(int row, double value)
    {
    ...
    private void AddUpper(int row, double value)
    {
    ...
    private void AddOff(int row, double value)
    {
    ...
    public override void ForwardSubstitute(IVector<double> solution)
    {
        ...
        for (var element = Vector.GetFirstInVector(); element != null; element = element.Below)
        {
            ...
            if (row >= 0 && row < size)
                _intermediate[_rowPosition[row]] = element.Value * _rowScaleReciprocal[row];
        }
        ...
        for (int block = Symbolic.Blocks - 1; block >= 0; block--)
        {
            ...
            {
                double value = _intermediate[k] / _diagonal[k];
    ...
    public override void BackwardSubstituteTransposed(IVector<double> solution)
    {
        ...
        for (int k = 0; k < size; k++)
        {
            ...
            solution[Row.Reverse(row + 1)] = _intermediate[k] * _rowScaleReciprocal[row];
        }
    }
    ...
    private int PrepareSolve(IVector<double> solution)
    {
