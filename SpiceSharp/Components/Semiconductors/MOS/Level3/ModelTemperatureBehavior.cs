        ...
        void ITemperatureBehavior.Temperature()
        {
            ...
            if (Parameters.SubstrateDoping.Given)
            {
                if (Parameters.SubstrateDoping * 1e6 /* (cm**3 / m**3) */ > 1.45e16)
                {
                    ...
                    {
                        Parameters.Phi.RawValue = 2 * VtNominal * Math.Log(Parameters.SubstrateDoping * 1e6 /* (cm^3/m^3) */  / 1.45e16);
                        Parameters.Phi.RawValue = Math.Max(0.1, Parameters.Phi);
                    }
                    var fermis = Parameters.MosfetType * 0.5 * Parameters.Phi;
                    var wkfng = 3.2;
                    if (!Parameters.GateType.Given)
                        Parameters.GateType.RawValue = 1;
                    if (!Parameters.GateType.RawValue.Equals(0.0))
                    {
                    ...
                    {
                        Parameters.Gamma.RawValue = Math.Sqrt(2 * EpsilonSilicon * Constants.Charge * Parameters.SubstrateDoping * 1e6 /* (cm**3 / m**3) */) /
