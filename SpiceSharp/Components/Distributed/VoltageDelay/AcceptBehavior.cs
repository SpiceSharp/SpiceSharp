...
namespace SpiceSharp.Components.DelayBehaviors
{
    public class AcceptBehavior : BaseAcceptBehavior
    {
        ...
        private double _oldSlope, _reltol, _abstol;
        ...
        {
            // Get tolerances
            if (provider.TryGetParameterSet("model", out ModelBaseParameters mbp))
            {
                _reltol = mbp.RelativeTolerance;
                _abstol = mbp.AbsoluteTolerance;
            }
            else
            {
                _reltol = 1.0;
        ...
        public override void Probe(TimeSimulation simulation)
        {
            ...
            _tran.DelayedSignal.Probe(simulation.Method.Time, breakpoint);
        }
        ...
        public override void Accept(TimeSimulation simulation)
        {
            ...
            // Calculate the current slope of the input, and decide if we should add a breakpoint
            if (simulation.Method is IBreakpoints method)
            {
                if (_wasBreak || method.Break)
                {
                    ...
                    var slope = method.Time.Equals(0.0)
                        ? 0.0
                        : (_tran.DelayedSignal.GetValue(0, 0) - _tran.DelayedSignal.GetValue(1, 0)) /
                          (_tran.DelayedSignal.GetTime(0) - _tran.DelayedSignal.GetTime(1));

                    // The previous point was a breakpoint, let's see if we need to add another breakpoint
                    if (_wasBreak)
                    {
                        var tol = _reltol * Math.Max(Math.Abs(_oldSlope), Math.Abs(slope)) + _abstol;
                        if (Math.Abs(slope - _oldSlope) > tol)
                            method.Breakpoints.SetBreakpoint(_tran.DelayedSignal.GetTime(1) + _tran.DelayedSignal.Delay);
                    }
            ...
            _tran.DelayedSignal.AcceptProbedValues();
        }
    }
}
