...
namespace SpiceSharp.NewSparse
{
    ...
    public sealed class ComplexElement : Element<Complex>
    {
        ...
        public override string ToString()
        {
            ...
            if (Real.Equals(0))
                return "{0}j".FormatString(Imaginary);
            if (Imaginary > 0)
                return "{0}+{1}j".FormatString(Real, Imaginary);
            return "{0}-{1}j".FormatString(Real, -Imaginary);
        }
        ...
        public override string ToString(string format, IFormatProvider formatProvider)
        {
            ...
            if (Real.Equals(0))
                return Imaginary.ToString(format, formatProvider) + "j";
            if (Imaginary > 0)
                return Real.ToString(format, formatProvider) + "+" + Imaginary.ToString(format, formatProvider) + "j";
            return Real.ToString(format, formatProvider) + "-" + (-Imaginary).ToString(format, formatProvider) + "j";
        }
    }
}
