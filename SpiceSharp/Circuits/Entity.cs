...
namespace SpiceSharp.Circuits
{
    ...
    /// </remarks>
    public abstract class Entity : ICloneable, ICloneable<Entity>, IParameterSet
    {
        ...
        public virtual void CreateBehaviors(ISimulation simulation, IEntityCollection entities)
        {
            ...
            {
                // Skip creating behaviors that are already implemented (eg. by inheritance)
                ...
                if (factories.TryGetValue(types[i], out var factory))
                {
                    ...
                    {
                        var newParam = LinkParameters ? p.Value : p.Value.Clone();
                        newParam.CalculateDefaults();
                        behavior.Parameters.Add(p.Key, newParam);
                    }
            ...
            // Bind the newly created behaviors to the simulation
            BindBehaviors(newBehaviors, simulation, entities);
        }
        ...
        protected virtual void BindBehaviors(IEnumerable<IBehavior> behaviors, ISimulation simulation, IEntityCollection entities)
        {
            simulation.ThrowIfNull(nameof(simulation));
            var context = new BindingContext(simulation, Name);
