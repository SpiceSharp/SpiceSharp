...
{
    public class BSIM3v23 : CircuitComponent<BSIM3v23>
    {
        /// <summary>
        ...
        /// </summary>
        public void SetModel(BSIM3v23Model model) => Model = model;
        ...
        public BSIM3v23(string name) : base(name)
        {
        }
        ...
        {
            var model = Model as BSIM3v23Model;
            pParam = null;
        ...
        public override void Temperature(Circuit ckt)
        {
            ...
            Tuple<double, double> size = new Tuple<double, double>(BSIM3w, BSIM3l);
            ...
            else
            {
                ...
                if (pParam.BSIM3leff <= 0.0)
                    throw new CircuitException($"BSIM3v23: mosfet {Name}, model {model.Name}: Effective channel length <= 0");
                pParam.BSIM3weff = BSIM3w - 2.0 * pParam.BSIM3dw;
                if (pParam.BSIM3weff <= 0.0)
                    throw new CircuitException($"BSIM3v23: mosfet {Name}, model {model.Name}: Effective channel width <= 0");

                pParam.BSIM3leffCV = BSIM3l - 2.0 * pParam.BSIM3dlc;
                if (pParam.BSIM3leffCV <= 0.0)
                    throw new CircuitException($"BSIM3v23: mosfet {Name}, model {model.Name}: Effective channel length for C-V <= 0");

                pParam.BSIM3weffCV = BSIM3w - 2.0 * pParam.BSIM3dwc;
                if (pParam.BSIM3weffCV <= 0.0)
                    throw new CircuitException($"BSIM3v23: mosfet {Name}, model {model.Name}: Effective channel width for C-V <= 0");
                ...
                if (BSIM3checkModel())
                    throw new CircuitException($"Fatal error(s) detected during BSIM3v3.2 parameter checking for {Name} in model {model.Name}");
        ...
        public override void Load(Circuit ckt)
        {
            ...
            /* BSIM3v23 thermal noise Qinv calculated from all capMod 
        ...
        {
            BSIM3v23Model model = (BSIM3v23Model)Model;
            bool Fatal_Flag = false;
            using (StreamWriter sw = new StreamWriter("b3v3check.log", true))
            {
                sw.WriteLine("BSIM3v3.2.3 Parameter Checking.");
                if (model.BSIM3version != "3.2.3")
                {
                    sw.WriteLine("Warning: This model is BSIM3v3.2.3; you specified a wrong version number.");
                    CircuitWarning.Warning(this, "Warning: This model is BSIM3v3.2.3; you specified a wrong version number.");
                }
